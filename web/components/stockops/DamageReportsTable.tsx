...
import DamageReportDetailSheet from './DamageReportDetailSheet';
import type { DamageReportListItem } from '@/types/stockops';

export default function DamageReportsTable() {
  const { activeBusiness } = useBusiness();
  const searchParams = useSearchParams();
  const router = useRouter();

  const [selected, setSelected] = useState<DamageReportListItem | null>(null);
  ...
  const { data, isLoading, isError } =
    clientTrpc.stockops.getDamageReports.useQuery(
      {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="text-muted-foreground text-sm">
          ...
          {Math.min(pageNumber * pageSize, total)} of {total} reports
        </div>
