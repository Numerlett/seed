...
import StockTransferDetailSheet from './StockTransferDetailSheet';
...
export default function StockTransfersTable() {
  ...
  const [selected, setSelected] = useState<StockTransferListItem | null>(null);
  ...
  const { data, isLoading, isError } =
    clientTrpc.stockops.getStockTransfers.useQuery(
      {
    ...
    <div className="space-y-4">
                ...
                >
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1 text-sm">
                      <span>{item.sourceWarehouse?.name || '—'}</span>
                      <ArrowRight className="h-3 w-3" />
                      <span>{item.destWarehouse?.name || '—'}</span>
                    </div>
                  </TableCell>
      ...
      <div className="flex items-center justify-between">
        <div className="text-muted-foreground text-sm">
          ...
          {Math.min(pageNumber * pageSize, total)} of {total} transfers
        </div>
