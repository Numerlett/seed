...
import type { PurchaseReturnListItem } from '@/types/returns';

interface PurchaseReturnDetailSheetProps {
  returnItem: PurchaseReturnListItem | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export default function PurchaseReturnDetailSheet({
  returnItem,
  open,
  onOpenChange,
}: PurchaseReturnDetailSheetProps) {
  const { activeBusiness } = useBusiness();
  const utils = clientTrpc.useUtils();

  const { data: detail, isLoading } =
    clientTrpc.returns.getPurchaseReturnById.useQuery(
      { id: returnItem?.id || '', businessId: activeBusiness?.id || '' },
      { enabled: !!returnItem?.id && !!activeBusiness?.id && open },
    );

  const confirmMutation = clientTrpc.returns.confirmPurchaseReturn.useMutation({
    onSuccess: () => {
      toast.success('Purchase return confirmed — stock updated');
      utils.returns.getPurchaseReturns.invalidate();
      utils.returns.getPurchaseReturnById.invalidate();
    },
    onError: (err) => toast.error(err.message),
  });

  const cancelMutation = clientTrpc.returns.cancelPurchaseReturn.useMutation({
    onSuccess: () => {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-2xl">
        <SheetHeader>
          <SheetTitle>
            Purchase Return {returnItem?.documentNumber || ''}
          </SheetTitle>
          <SheetDescription>Purchase return details</SheetDescription>
        </SheetHeader>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                {detail.grn && (
                  <div>
                    ...
                    <p className="font-medium">{detail.grn.documentNumber}</p>
                  </div>
                )}
                {detail.reason && (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This will deduct stock from the warehouse (returning
                          goods to supplier). This action cannot be undone.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
