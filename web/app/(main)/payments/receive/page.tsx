...
export default function ReceivePaymentPage() {
  ...
  const { data: parties } = clientTrpc.party.getPartiesByBusinessId.useQuery(
    { businessId: activeBusiness?.id ?? '', partyType: 'CUSTOMER' },
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <PageTitle title="Receive Payment" subtitle="Record money received from a customer" />
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Select onValueChange={(v) => form.setValue('partyId', v)}>
                ...
                <SelectContent>
                  {parties?.parties?.map((p) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input placeholder="UTR, UPI ref, cheque no..." {...form.register('reference')} />
            </div>
